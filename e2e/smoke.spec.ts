...
test.describe("public site", () => {
  test("home loads and links to projects", async ({ page }) => {
    ...
    await page
      .getByRole("link", { name: /voir mes projets/i })
      .first()
      .click();
    await page.waitForURL(/\/projects(\?|$)/);
  });

  test("navbar 'Profil' group reaches the academic path", async ({ page }) => {
  ...
  test("contact form rejects an invalid submission client-side", async ({ page }) => {
    ...
    await expect(
      page.getByText(/nom trop court|message trop court/i).first(),
    ).toBeVisible();
  });
});
